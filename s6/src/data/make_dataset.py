...
def mnist(input_filepath, output_filepath, batch_size: int = 8):
    ...
    input_folder_v2 = f"{input_filepath}/corruptmnist_v2/"
    output_folder = f"{output_filepath}/corruptmnist/"
    

    train_files = [f"train_{i}.npz" for i in range(5)]
    train_files_v2 = [f"train_{i}.npz" for i in range(5, 8)]
    ...
    for file in train_files_v2:
        with np.load(input_folder_v2 + file) as data:
            x_train.extend(data["images"])
            y_train.extend(data["labels"])
    ...
    train = DataLoader(
        TensorDataset(norm_transform(x_train), y_train),
        shuffle=True,
        batch_size=batch_size,
    )

    test = DataLoader(
        TensorDataset(norm_transform(x_test), y_test),
        shuffle=False,
        batch_size=batch_size,
    )
